...
if [ -d "/boot/loader/entries" ]; then # systemd-boot
  ...
  if ! grep -q splash /etc/cmdline.d/*.conf; then
    # Need splash, create the anarchy file
    echo "splash" | sudo tee -a /etc/cmdline.d/anarchy.conf
  fi
  if ! grep -q quiet /etc/cmdline.d/*.conf; then
    # Need quiet, create or append the anarchy file
    echo "quiet" | sudo tee -a /etc/cmdline.d/anarchy.conf
  fi
...
if [ "$(plymouth-set-default-theme)" != "anarchy" ]; then
  ...
  sudo plymouth-set-default-theme -R anarchy
fi
...
if [ ! -f /etc/systemd/system/anarchy-seamless-login.service ]; then
  cat <<EOF | sudo tee /etc/systemd/system/anarchy-seamless-login.service
[Unit]
Description=Anarchy Seamless Auto-Login
Documentation=https://github.com/CobyPowers/anarchy
...
# Enable anarchy-seamless-login.service only if not already enabled
if ! systemctl is-enabled anarchy-seamless-login.service | grep -q enabled; then
  sudo systemctl enable anarchy-seamless-login.service
fi
